...
export default function Chat(){
 ...
 await carregar();timer=setInterval(carregar,2500)})();return()=>clearInterval(timer)},[matchId])
 useEffect(()=>{end.current?.scrollIntoView({behavior:'smooth'})},[msgs])
 async function carregar(){const{data,error}=await s.from('messages').select('id,sender_id,body,created_at').eq('match_id',matchId).order('created_at',{ascending:true});if(!error){setMsgs(data||[]);setStatus('')}}
 async function enviar(e:FormEvent){e.preventDefault();const text=body.trim();if(!text||!userId)return;setBody('');const{error}=await s.from('messages').insert({match_id:matchId,sender_id:userId,body:text});if(error){setStatus('Não foi possível enviar a mensagem.');setBody(text);return}await carregar()}
 ...
 {msgs.length?msgs.map(m=><div key={m.id} style={{alignSelf:m.sender_id===userId?'flex-end':'flex-start',maxWidth:'78%',padding:'10px 13px',borderRadius:16,background:m.sender_id===userId?'#7c203b':'#25151a',lineHeight:1.4}}>{m.body}</div>):<p style={{textAlign:'center',color:'#9f8e93',margin:'auto'}}>Comece a conversa. Tudo começa com uma mensagem.</p>}<div ref={end}/></div>
